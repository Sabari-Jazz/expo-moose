...
- Categorizes inverters by status (green, red, offline)
...
- Else if ANY offline inverters: system status = "offline"  
- Else (all green): system status = "green"
...
def get_system_inverters(system_id: str) -> List[str]:
    """Get all inverter device IDs for a specific system"""
    try:
        ...
        for item in response.get('Items', []):
            ...
            if device_id:
                inverter_ids.append(device_id)
        
        logger.info(f"Found {len(inverter_ids)} inverters for system {system_id}")
        return inverter_ids
    ...
    try:
        ...
        for item in response.get('Items', []):
            ...
            if status == 'green':
                ...
                # Unknown status, treat as offline
                offline_inverters.append(device_id)
...
def determine_system_status(green_inverters: List[str], red_inverters: List[str], offline_inverters: List[str]) -> str:
...
def update_system_status(system_id: str, green_inverters: List[str], red_inverters: List[str], offline_inverters: List[str]) -> bool:
    """Update system status in DynamoDB"""
    try:
        # Determine overall system status
        overall_status = determine_system_status(green_inverters, red_inverters, offline_inverters)
        ...
        current_offline = set(current_status_record.get('OfflineInverters', []) or [])
        current_overall = current_status_record.get('status', 'offline')
        
        # Convert new lists to sets
        new_green = set(green_inverters)
        new_red = set(red_inverters)
        new_offline = set(offline_inverters)
        
        # Check if there are any changes
        if (current_green == new_green and 
            current_red == new_red and 
            current_offline == new_offline and 
        ...
        total_inverters = len(green_inverters) + len(red_inverters) + len(offline_inverters)
        ...
        logger.info(f"  Green: {len(green_inverters)}, Red: {len(red_inverters)}, Offline: {len(offline_inverters)}")
